...
2. **Painel interno (staff)**: usa o cliente Supabase com *anon key* + sessão autenticada (`supabase.auth`). Não há filtro manual de `client_id` em lugar nenhum do front — a RLS `tenant_isolation` (`0001_init.sql`) é a única barreira, baseada no claim `app_metadata.client_id` do JWT. Esse claim só pode ser setado pela service role (`scripts/create-staff-user.mjs`, via `auth.admin.createUser`), nunca pelo próprio usuário.
